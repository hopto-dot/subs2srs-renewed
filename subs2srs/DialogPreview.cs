...
namespace subs2srs
{
  ...
  public partial class DialogPreview : Form
  {
    ...
    private bool combineAdjacentSubtitles = false;
    ...
    private void generatePreview()
    {
      ...
      try
      {
        ...
        // workerVars.combineAdjacentSubtitles = combineAdjacentSubtitles;
    ...
    private List<List<InfoCombined>> combineAdjacentLines(WorkerVars workerVars, DialogProgress dialogProgress)
    {
        if (!Settings.Instance.CombineAdjacentLines)
        {
            return workerVars.CombinedAll;
        }

        List<List<InfoCombined>> combinedAll = workerVars.CombinedAll;

        for (int i = 0; i < combinedAll.Count; i++)
        {
            List<InfoCombined> combinedList = combinedAll[i];
            List<InfoCombined> newCombinedList = new List<InfoCombined>();

            for (int j = 0; j < combinedList.Count; j++)
            {
                InfoCombined currentLine = combinedList[j];

                if (j < combinedList.Count - 1 && (currentLine.Subs1.Text.TrimEnd().EndsWith("から") || currentLine.Subs1.Text.TrimEnd().EndsWith("けど")))
                {
                    InfoCombined nextLine = combinedList[j + 1];
                    InfoCombined combinedLine = new InfoCombined(
                        new InfoLine(currentLine.Subs1.StartTime, nextLine.Subs1.EndTime, currentLine.Subs1.Text.TrimEnd() + " " + nextLine.Subs1.Text.TrimStart()),
                        new InfoLine(currentLine.Subs2.StartTime, nextLine.Subs2.EndTime, currentLine.Subs2.Text.TrimEnd() + " " + nextLine.Subs2.Text.TrimStart())
                    );

                    newCombinedList.Add(combinedLine);
                    j++;
                }
                else
                {
                    newCombinedList.Add(currentLine);
                }
            }

            combinedAll[i] = newCombinedList;
        }
    ...
    private void bw_DoWork(object sender, DoWorkEventArgs e)
    {
      ...
      try
      {
        ...
        if (Settings.Instance.CombineAdjacentLines == true)
        {
            combinedAll = combineAdjacentLines(workerVars, dialogProgress); // added
        }
